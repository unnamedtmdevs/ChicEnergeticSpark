    ...
    var body: some View {
        ZStack {
            if isFetched == false {
                ...
                if isBlock == true {
                    // Сервер недоступен или ответил ошибкой - показываем обычное приложение (игры)
                    ZStack {
                        if showOnboarding {
                    ...
                    // Сервер ответил 200 с контентом или 3xx - показываем WebView
                    WebSystem()
                }
            }
        }
        ...
        session.dataTask(with: request) { data, response, error in
            ...
            DispatchQueue.main.async {
                ...
                if let error = error {
                    ...
                    print("Server unavailable, showing games")
                    self.isBlock = true
                    self.isFetched = true
                    return
                }
                ...
                if let httpResponse = response as? HTTPURLResponse {
                    ...
                    if httpResponse.statusCode == 200 {
                        ...
                        if contentLength == "0" || !hasContent {
                            ...
                            print("🚫 Empty response (do nothing): Showing games")
                            self.isBlock = true
                            self.isFetched = true
                        } else {
                            // Есть контент = успех
                        ...
                        print("🚫 Error code \(httpResponse.statusCode): Showing games")
                        self.isBlock = true
                        self.isFetched = true
                    }
                    ...
                    print("❌ No HTTP response: Showing games")
                    self.isBlock = true
                    self.isFetched = true
                }
            }
